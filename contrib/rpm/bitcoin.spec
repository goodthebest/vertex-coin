...
%{_sbindir}/semanage port -a -t bitcoin_port_t -p tcp 5332
%{_sbindir}/semanage port -a -t bitcoin_port_t -p tcp 5444
%{_sbindir}/semanage port -a -t bitcoin_port_t -p tcp 15332
%{_sbindir}/semanage port -a -t bitcoin_port_t -p tcp 15444
...
if [ $1 -eq 0 ]; then
	if [ `%{_sbindir}/sestatus |grep -c "disabled"` -eq 0 ]; then
	%{_sbindir}/semanage port -d -p tcp 5332
	%{_sbindir}/semanage port -d -p tcp 5444
	%{_sbindir}/semanage port -d -p tcp 15332
	%{_sbindir}/semanage port -d -p tcp 15444
	for selinuxvariant in %{selinux_variants}; do
